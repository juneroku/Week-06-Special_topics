...
Docker ติดตั้งง่าย ไม่ต้องจัดการ dependency เอง เหมาะกับทีมงานหลายคน, ส่วน native ต้องตั้งค่ามือแต่ยืดหยุ่นกว่า

2. **Build Process**: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary
รัน idf.py build → CMake สร้าง build system → Compile → Link → ได้ binary พร้อม flash

3. **CMake Files**: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?
CMakeLists.txt ที่ root กำหนด project, ของแต่ละ component กำหนด source/build → Docker ใช้เหมือนกันเพราะ build ใน container

4. **Git Ignore**: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?
ช่วยกันไฟล์ build, log, config ส่วนตัวไม่ให้ถูก push ขึ้น Git ทำให้โปรเจกต์สะอาด

5. **Container Persistence**: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?
ไฟล์ใน container จะหายเมื่อ restart แต่ไฟล์ใน bind mount เช่น source code จะยังอยู่

6. **Development Workflow**: เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system
Docker ทำงานเสถียร เหมาะกับทีม, Native เร็วกว่าและปรับแต่งง่ายแต่ดูแลระบบเอง
