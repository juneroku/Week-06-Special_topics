...
เพื่อทำให้โครงสร้างโปรเจคอ่านง่าย ดูแลรักษาง่าย และสามารถพัฒนาเป็นส่วนๆ (โมดูล) ได้ เช่น ถ้า sensor มีปัญหา ก็แก้เฉพาะ sensor.c ไม่กระทบส่วนอื่น 

2. **CMakeLists.txt Management**: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?
ต้องนำชื่อไฟล์ .c ที่เพิ่ม เข้าไปในฟิลด์ SRCS ของคำสั่ง idf_component_register() ในไฟล์ main/CMakeLists.txt เพื่อให้ระบบ build รู้จักและ compile ไฟล์ใหม่ด้วย

3. **Header Files**: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?
เป็นที่ประกาศ prototype ของฟังก์ชัน, struct, และตัวแปร
ช่วยให้ไฟล์อื่นๆ สามารถ #include มาใช้งานฟังก์ชันจากไฟล์อื่นได้

4. **Include Directories**: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?
เพื่อบอก CMake ว่า header files (.h) อยู่ในโฟลเดอร์ไหนบ้าง เวลา #include "sensor.h" หรือ "display.h" จะได้ค้นหาเจอ ไม่เช่นนั้น compiler จะไม่รู้ path ของไฟล์

5. **Git Ignore**: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?
ใช้บอก Git ว่าไม่ต้อง track ไฟล์บางชนิด เช่น โฟลเดอร์ build/, ไฟล์ binary (.bin, .elf), log, temporary files — ช่วยให้ repository สะอาด เก็บเฉพาะ source code และ config ที่จำเป็น

6. **Task Management**: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?
ทำให้ LED สามารถกระพริบ (toggle) ในพื้นหลังแบบอัตโนมัติ ไม่บล็อก loop หลัก ช่วยให้ main loop ยังสามารถอ่าน sensor/ แสดงผล display ได้

7. **Code Organization**: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?
แก้ไขและ debug ง่ายตามฟังก์ชันของโมดูล
นำกลับมาใช้ซ้ำในโปรเจคอื่นได้
แบ่งงานให้ทีมทำเป็นส่วนๆ 
โครงสร้างโปรเจคเป็นระบบและขยายง่ายในอนาคต
...
- การทำงาน: แสดงผล log ของ sensor (init, read, status) ทุก ๆ loop

**ขั้นตอนที่ 2 (เพิ่ม display.c):**
- จำนวนไฟล์ source: 3
- ขนาด binary: 170,450 bytes
- การทำงาน:  แสดง message และข้อมูล sensor ผ่าน display module

**ขั้นตอนที่ 3 (เพิ่ม led.c):**
- จำนวนไฟล์ source:  4
- ขนาด binary: 175,820 bytes
- การทำงาน: LED กะพริบทุก 3 วินาที พร้อมแสดง status ON/OFF ผ่าน display
